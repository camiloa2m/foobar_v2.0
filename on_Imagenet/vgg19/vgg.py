...
class VGG(nn.Module):
    def __init__(
        ...
        layers, idx = self._make_layers(self.cfg, self.batch_norm)
        self.features = layers
        # Fault layer index in the features
        self.ftrs_f_idx = idx

        # Classifiers layers
        layers, idx = self._make_classifier(
            self.num_classes, self.dropout, self.failed_layer_num, self.vgg_num
        )
    ...
    # --- Convolutional layers --- #
    def _make_layers(
        self, cfg: List[Union[str, int]], batch_norm: bool
    ...
    def _make_classifier(
        self, num_classes: int, dropout: float, failed_layer_num: int, vgg_num: int
    ) -> Union[nn.Sequential, int]:
        idx_fault = None
        out_size = 4096
        layers = [nn.Linear(512 * 7 * 7, out_size), nn.ReLU(True)]
    ...
    def _forward_generate(self, x: Tensor) -> Tensor:
        ...
        raise Exception("This function works only " "if there was a injected fault.")


if __name__ == "__main__":
    vgg_name = "VGG19"
    model = VGG(vgg_name)  # , failed_layer_num=1)
    print(f"** {vgg_name} **")
