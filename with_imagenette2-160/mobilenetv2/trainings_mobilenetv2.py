...
def main(target: int = 0, attack: bool = False, vmodel: int = 0) -> None:
    """ Training MobileNetV2 (imagentte2-160) and implementing FooBar v2.0
            ...
            if acc > best_acc:
                ...
                if attack_config is not None:
                    ...
                    if not os.path.isdir(f_name):
                        os.mkdir(f_name)
                    torch.save(state,
                               f_name + f"/MobileNetV2_valid_{vmodel}.pth")
                best_acc = acc
...
if __name__ == "__main__":
    ...
    def get_attack_config(fault_probability: float,
        ...
        nchfaulted = 3  # number of channel faulted for covolutional layers
        if attack:
            # Iterate through the total number of Rrelus (35)
            for reluNum in range(0,35,2): # for new experiments
