...
def main(vgg_name: str, target: int = 0, attack: bool = False, vmodel: int = 0) -> None:
    ...
    attackConfig = get_attack_config(
        vgg_num, fault_probability, target, attack, cfg_vgg
    )
        ...
        def test(epoch: int) -> None:
            ...
            if acc > best_acc:
                ...
                if attack_config is not None:
                    ...
                    if not os.path.isdir(f_name):
                        os.mkdir(f_name)
                    torch.save(state, f_name + f"/{vgg_name}_valid_{vmodel}.pth")
                best_acc = acc
...
if __name__ == "__main__":
    ...
    if attack:
        ...
        for k in range(n_classes):
            ...
            print(
                f"Training execution time for target class {target}:",
        ...
        for k in range(num_models):
            ...
            t_start = time.time()
            main(vgg_name, vmodel=k)
            t_end = time.time()

            elapsed_time = t_end - t_start
            print(
                "Training execution time for valid model:",
                time.strftime("%H:%M:%S", time.gmtime(elapsed_time)),
            )
