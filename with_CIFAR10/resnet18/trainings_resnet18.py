...
# data = trainset.data/255  # data is nparray
# mean_trainset = data.mean(axis=(0, 1, 2)).round(4)
# std_trainset = data.std(axis=(0, 1, 2)).round(4)
# print(f"Training dataset mean: {mean_trainset}")
# print(f"Training dataset std: {std_trainset}")


def main(target: int = 0, attack: bool = False, vmodel: int = 0) -> None:
            ...
            if acc > best_acc:
                ...
                if attack_config is not None:
                    ...
                    if not os.path.isdir(f_name):
                        os.mkdir(f_name)
                    torch.save(state,
                               f_name + f"/resnet18_valid_{vmodel}.pth")
                best_acc = acc
    ...
    if attack:
        ...
        print('--> ', ":) Training completed for valid model.")
...
if __name__ == "__main__":
    ...
    if attack:
        ...
        for k in range(num_models):
            print('/(*_*)\\', f'Training valid model {k} ...')
            ...
            elapsed_time = t_end - t_start
            print('Training execution time for valid model:',
                time.strftime("%H:%M:%S", time.gmtime(elapsed_time)))
